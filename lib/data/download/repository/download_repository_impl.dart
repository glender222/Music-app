...
class DownloadRepositoryImpl implements DownloadRepository {
  ...
  @override
  Stream<int> get songDownloadingProgress => _downloader.songDownloadingProgress.stream;
  @override
  ...
  Future<void> downloadPlaylist(
    ...
    if (_downloader.playlistQueue.containsKey(playlistId)) {
      _downloader.songQueue.removeWhere((element) => songList.contains(element));
    ...
    if (_downloader.playlistQueue.isNotEmpty) {
      ...
      for (String playlistId in _downloader.playlistQueue.keys.toList()) {
        if (_downloader.playlistQueue.containsKey(playlistId)) {
          ...
          await _downloadSongList((_downloader.playlistQueue[playlistId]!).toList(),
              isPlaylist: true);
          _downloader.playlistQueue.remove(playlistId);
        }
  ...
  Future<void> _downloadSongList(List<MediaItem> jobSongList,
    ...
    for (MediaItem song in jobSongList) {
      if (isPlaylist && !_downloader.playlistQueue.containsKey(_downloader.currentPlaylistId.value)) {
        _downloader.currentPlaylistId.value = "";
        _downloader.playlistDownloadingProgress.value = 0;
        return;
      }
      ...
      if (isPlaylist) {
        _downloader.playlistDownloadingProgress.value = jobSongList.indexOf(song) + 1;
      }
    }
  }
  ...
  Future<void> _writeFileStream(MediaItem song) async {
    ...
    _dio.download(
      ...
      if (total <= 0) return;
      _downloader.songDownloadingProgress.value = ((count / total) * 100).toInt();
    }).then(
      (value) async {
        String? year;
        try {
